/*
	...
	This file has been extracted from the MPAndroidChart library. By putting it here,
	we ensure that this ChartData class will override the one of the MPAndroidChart library.

	We do this to fix a small issue in the original file: hidden DataSets are taken into
	account when computing the minY and maxY, so graphics are not rescaled properly when
	hidding or showing DataSets (look for the "SYL"  string below to see the fix).

*/

// DO NOT CHANGE THIS PACKAGE NAME. It ensures that this ChartData class will override
// the one of the MPAndroidChart library
...
/*
		...
		if (firstLeft != null) {
			...
			for (T dataSet : mDataSets) {
				// SYL: added "dataSet.isVisible()"
				if (dataSet.isVisible() && dataSet.getAxisDependency() == AxisDependency.LEFT) {
					if (dataSet.getYMin() < mLeftAxisMin)
						mLeftAxisMin = dataSet.getYMin();
		...
		if (firstRight != null) {
			...
			for (T dataSet : mDataSets) {
				// SYL: added "dataSet.isVisible()"
				if (dataSet.isVisible() && dataSet.getAxisDependency() == AxisDependency.RIGHT) {
					if (dataSet.getYMin() < mRightAxisMin)
						mRightAxisMin = dataSet.getYMin();
